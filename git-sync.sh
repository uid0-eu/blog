...
function cleanup {
#git gc
if [ "$cleanup" == "true" ] && [ $(git log | grep -c "commit ") -gt 10 ]; then
id=$(git rev-parse HEAD~5)
git checkout --orphan temp $id      # checkout to the status of the git repo at commit f; creating a branch named "temp"
git commit -m "new root commit"     # create a new commit that is to be the new root commit
git rebase --onto temp $id master   # now rebase the part of history from <f> to master onthe temp branch
git branch -D temp
...
#git config --global user.name "bastian"
#git config --global user.email bastian.brickwedde@sanimed.de
